	...
	)
{
	...
	CHECK_CUDA(FORWARD::preprocess<NUM_CHANNELS>(
		P, D, M,
		means3D,
		(glm::vec2*)scales,
		scale_modifier,
...
void CudaRasterizer::Rasterizer::backward(
	...
	char* binning_buffer,
	char* img_buffer,
	const float* dL_dcolor_pix,
	const float* dL_dextra_pix,
	...
	bool debug)
{
	...
	const float* transMat_ptr = (transMat_precomp != nullptr) ? transMat_precomp : geomState.transMat;
	CHECK_CUDA(BACKWARD::render<NUM_CHANNELS>(
		tile_grid,
		block,
		imgState.ranges,
		binningState.point_list,
	...
	// const float* transMat_ptr = (transMat_precomp != nullptr) ? transMat_precomp : geomState.transMat;
	CHECK_CUDA(BACKWARD::preprocess<NUM_CHANNELS>(P, D, M,
		(float3*)means3D,
		radii,
		shs,
		geomState.clamped,
...
#define X(N) \
	template int CudaRasterizer::Rasterizer::forward<N>( \
		std::function<char* (size_t)> geometryBuffer, \
		std::function<char* (size_t)> binningBuffer, \
		std::function<char* (size_t)> imageBuffer, \
		const int P, int D, int M, \
