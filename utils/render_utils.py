...
def generate_ellipse_path(poses: np.ndarray, n_frames: int = 120, const_speed: bool = True, n_rots=1, z_variation: float = 0.0, z_phase: float = 0.0) -> np.ndarray:
    ...
    def get_positions(theta):
        ...
        return np.stack(
            [
                ...
                z_variation * (z_low[2] + (z_high - z_low)[2] * (np.cos(theta / n_rots + 2 * np.pi * z_phase) * 0.5 + 0.5)),
            ],
            -1,
        )
...
def generate_spiral_path(
    ...
    radii=[0.0, 0.0, 0.0],
    offset=[0, 0.0, 0.0],
    pose_slice=slice(None),
) -> np.ndarray:
    """Calculate a forward-facing spiral path (DTU-style) for rendering."""
    # Get radii for spiral path using percentile of camera positions.
    radii = np.concatenate([radii, [1.0]])
    offset = np.concatenate([offset, [0.0]])

    # Generate poses for spiral path.
    render_poses = []
    cam2world = average_pose(poses[pose_slice])
    up = poses[:, :3, 1].mean(0)
    focus_point = focus_point_fn(poses)
    for theta in np.linspace(0.0, 2.0 * np.pi * n_rots, n_frames, endpoint=False):
        t = offset + radii * np.array(
            [np.cos(theta), -np.sin(theta), -np.cos(theta * zrate), 1.0],
            dtype=np.float32,
        )
        position = cam2world @ t
        render_poses.append(viewmatrix(position - focus_point, up, position))

    return np.stack(render_poses, axis=0)


def generate_path(viewpoint_cameras, n_frames=480, zoom_factor=1.0, path_type: str = 'ellipse', path_kwargs: Optional[Mapping] = None):
    ...
    path_kwargs = dict(path_kwargs or {})

    invalid_keys = {'poses', 'n_frames'} & set(path_kwargs.keys())
    if invalid_keys:
        raise ValueError(f'path_kwargs cannot override reserved arguments: {sorted(invalid_keys)}')

    # generate new poses
    if path_type == 'ellipse':
        new_poses = generate_ellipse_path(poses=pose_recenter, n_frames=n_frames, **path_kwargs)
    elif path_type == 'spiral':
        new_poses = generate_spiral_path(poses=pose_recenter, n_frames=n_frames, **path_kwargs)
    else:
        raise ValueError(f'Unsupported path_type: {path_type}. Expected one of ["ellipse", "spiral"].')
    ...
    for c2w in new_poses:
        ...
        if zoom_factor != 1.0:
            cam.FoVx = 2 * np.arctan(np.tan(cam.FoVx * 0.5) / zoom_factor)
            cam.FoVy = 2 * np.arctan(np.tan(cam.FoVy * 0.5) / zoom_factor)
...
def create_videos(base_dir, input_dir, out_name, num_frames=480, fps=30):
    ...
    rgb_file = os.path.join(input_dir, 'renders', f'{idx_to_str(0)}.png')
    rgb_frame = load_img(rgb_file)
